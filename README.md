...
GOOGLE_CLOUD_PROJECT ... (GCP設定)
DOCUMENT_AI_PROCESSOR_ID ... (DocAI設定)
💻 開発とデプロイ
ローカル実行に関する注意 ⚠️
本システムは Google Cloud Tasks や LINE WORKS Webhook (署名検証)、Document AI などのクラウドサービスに高度に依存しています。 そのため、単純な python main.py だけでは完全な動作確認はできません。
