...
*   **Google Cloud Platform (GCP)** プロジェクト (Document AI, Cloud Run, Pub/Sub, Secret Manager有効化)
...
*   `GOOGLE_CLOUD_PROJECT`, `PUBSUB_TOPIC_NAME` ... (GCP設定)
*   `DOCUMENT_AI_PROCESSOR_ID` ... (DocAI設定)

## 💻 開発とデプロイ

### ローカル実行に関する注意 ⚠️
本システムは **Google Cloud Pub/Sub** や **LINE WORKS Webhook (署名検証)**、**Document AI** などのクラウドサービスに高度に依存しています。
